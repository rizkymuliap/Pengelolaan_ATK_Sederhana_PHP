...
<body>
    ...
    <section class="section-body">
        <h1 style="margin-Left:15%; ">PENGELOLAAN VENDOR ATK</h1>
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" style="margin-Left:60%; margin-Bottom:1%;">
            <i class="bi bi-plus-circle"></i> VENDOR ATK
        </button>
        <!-- Modal -->
          ...
          <div class="modal-dialog">
            <div class="modal-content" style="display:flex; justify-Content:center; align-items:center">
              <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Tambah Vendor ATK</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <form name="myform" action="./VendorATK/save_input.php" method="post">
                <div class="modal-body">
                    Nama Vendor    :  <br /><input type="text" name="nama_vendor" required> <br />
                    Alamat Vendor  :  <br /><input type="text" name="alamat_vendor" required> <br />
                    Telepon Vendor :  <br /><input type="text" name="telp_vendor" required> <br />
                    Email Vendor   :  <br /><input type="email" name="email_vendor" required> <br />
                </div>
                        ...
                        while(($line = fgets($handle)) !== false){
                            ...
                            echo "
                                <tr>
                                    ...
                                    <td>$arr_data[4]</td>
                                    <td>
                                      <button type='button' class='btn btn-success'><a class='none-decoration' href='./VendorATK/form_update.php?id_vendor=$arr_data[0]' ><i class='bi bi-pencil-square'></i></a></button>
                                      <form method='post'>
                                          <input type='hidden' name='id_vendor' value='$arr_data[0]'>
                                          <input type='hidden' name='nama_vendor' value='$arr_data[1]'>
                                          <input type='hidden' name='alamat_vendor' value='$arr_data[2]'>
                                          <input type='hidden' name='telp_vendor' value='$arr_data[3]'>
                                          <input type='hidden' name='email_vendor' value='$arr_data[4]'>
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($lines as $line) {
            ...
            if ($arr_data[0] === $id_vendor) {
                ...
                $new_line = "$id_vendor|$nama_vendor|$alamat_vendor|$telp_vendor|$email_vendor|0\n";
                $new_lines[] = $new_line;
                $found = true;
            } else {
        ...
        if ($found) {
            ...
            echo "<script>alert('Data telah dihapus.'); window.location='./Vendor.php';</script>";
        } else {
